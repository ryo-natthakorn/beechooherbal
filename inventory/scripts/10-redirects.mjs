...
// --- Cloudflare Pages -----------------------------------------------------------
// The same table in Cloudflare's format. BOTH files are emitted from this one source, so
// the two hosts can never disagree and switching between them is a DNS change rather than
// a migration. Cloudflare reads public/_redirects (Astro copies public/ into dist/);
// Vercel reads vercel.json and ignores _redirects, and vice versa, so they coexist.
//
// One rule per line: `<source> <destination> <status>`. Cloudflare's documented ceiling is
// 2,000 static redirects, and this table is well under it.
//
// Destinations stay percent-encoded, for the same reason as vercel.json: a raw UTF-8
// destination silently broke 149 rules on Vercel (see add() above). Cloudflare does not
// document its behaviour either way, so encoding is the safe default -- and `npm run
// url-check` against a real deploy is what actually settles it.
const cfLines = [
  '# GENERATED by inventory/scripts/10-redirects.mjs -- do not edit by hand.',
  '# Regenerate with `npm run redirects`. Mirrors vercel.json exactly.',
  ...rules.map((r) => `${r.source} ${r.destination} ${r.statusCode}`),
];
writeFileSync(path.join(ROOT, 'public', '_redirects'), cfLines.join('\n') + '\n');

const encoded = rules.filter((r) => r.source !== decodeURI(r.source)).length;
console.log(`wrote vercel.json + public/_redirects: ${rules.length} redirect(s)`);
