...
| **WP REST `posts` "500s for every variant"** | ❌ **STALE/OVERBROAD.** Only large page-1 collection queries 500 (`per_page=50`/`100&page=1`). `per_page=1&page=N` walked **all 54 posts successfully — including the alopecia post**. `x-wp-total=54` matches the post-sitemap. **Phase 4 can migrate every post via REST, one at a time.** ⚠⚠ **PARTLY OVERTURNED 2026-08-24:** the claim that this includes the alopecia post (id 1530) no longer holds — it 500s on `posts/1530?_embed`, on `posts?include=1530`, on its HTML page, and on any listing requesting `excerpt` (the excerpt is what breaks). Its body was recovered from a 2022 Wayback capture instead; see `inventory/scripts/13-fetch-wayback.mjs` and `src/data/blog.ts`. The other 53 posts did migrate via REST as described. |
