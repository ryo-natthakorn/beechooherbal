...
- Native video and desktop dropdown results are in `interactions.json`. A later mobile-menu keyboard attempt targeted the inner sr-only `span` via text locator and failed to press; retest the enclosing native `summary`, not the span. This is not evidence that the menu itself is broken. Earlier click navigation EN/TH passed.
